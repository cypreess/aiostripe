...
class ChargeTest(StripeResourceTest):
    async def test_charge_list_all(self):
        await aiostripe.Charge.list(created={'lt': NOW})
    ...
    async def test_charge_list_create(self):
        await aiostripe.Charge.create(idempotency_key='foo', **DUMMY_CHARGE)

        self.requestor_mock.request.assert_called_with('post', '/v1/charges',
                                                       DUMMY_CHARGE, {'Idempotency-Key': 'foo'})

    async def test_charge_list_retrieve(self):
        await aiostripe.Charge.retrieve('ch_test_id')

        self.requestor_mock.request.assert_called_with('get', '/v1/charges/ch_test_id',
                                                       {}, None)
    ...
    async def test_charge_update_dispute(self):
        ...
        self.requestor_mock.request.assert_called_with('post', '/v1/charges/ch_update_id/dispute',
                                                       {}, {'Idempotency-Key': 'foo'})
    ...
    async def test_charge_close_dispute(self):
        ...
        self.requestor_mock.request.assert_called_with('post', '/v1/charges/ch_update_id/dispute/close',
                                                       {}, {'Idempotency-Key': 'foo'})
    ...
    async def test_mark_as_fraudulent(self):
        ...
        self.requestor_mock.request.assert_called_with('post', '/v1/charges/ch_update_id',
                                                       {
    ...
    async def test_create_with_source_param(self):
        await aiostripe.Charge.create(amount=100, currency='usd', source='btcrcv_test_receiver')
