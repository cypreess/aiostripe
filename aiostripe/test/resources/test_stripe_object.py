...
class StripeObjectTests(StripeUnitTestCase):
    def test_initializes_with_parameters(self):
        obj = aiostripe.resource.StripeObject('foo', 'bar', myparam=5, yourparam='boo')

        self.assertEqual('foo', obj.id)
        self.assertEqual('bar', obj.api_key)

    def test_access(self):
        obj = aiostripe.resource.StripeObject('myid', 'mykey', myparam=5)
    ...
    def test_refresh_from(self):
        obj = aiostripe.resource.StripeObject.construct_from({
            'foo': 'bar',
            'trans': 'me',
        }, 'mykey')
    ...
    def test_refresh_from_nested_object(self):
        obj = aiostripe.resource.StripeObject.construct_from(SAMPLE_INVOICE, 'key')

        self.assertEqual(1, len(obj.lines.subscriptions))
        self.assertTrue(isinstance(obj.lines.subscriptions[0], aiostripe.resource.StripeObject))
    ...
    def test_repr(self):
        obj = aiostripe.resource.StripeObject('foo', 'bar', myparam=5)
    ...
    def test_deletion(self):
        obj = aiostripe.resource.StripeObject('id', 'key')
